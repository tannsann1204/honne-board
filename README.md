...
DB は `data/honne.db` に、投稿画像は `data/uploads/` に自動作成されます(gitignore 済み)。

デザインは黒基調 × ネオンカラー。カテゴリごとに配色が変わります。

## 機能

- 匿名投稿(ニックネーム任意・カテゴリ8種: 仕事/恋愛/家族/友人/お金/学校/健康/雑談)
- 写真投稿(JPEG/PNG/WebP/GIF、5MBまで。マジックバイト検証つきで `data/uploads/` に保存、`/uploads/[name]` で配信)
